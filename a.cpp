...
struct Zp
{
    ...
    Zp(uint32_t value_ = 0): value(value_)
    {
    }

    Zp& operator*=(const Zp& other)
    {
        value = (1ULL * value * other.value) % p;
...
std::vector<Zp> scan_message(uint32_t p)
{
    std::string buffer;
    ...
    {
        number *= 64;
        number += char_to_number(c);
    }

    std::vector<Zp> message;
    while (number > 0)
    {
        message.push_back(Zp(number % p));
        number /= p;
    }
...
std::vector<std::pair<Zp, Zp>> encrypt_el_gamal(const Zp& g, const Zp& s, const std::vector<Zp>& message)
{
    uint32_t n = g.get_ord();
    std::mt19937 rnd(std::chrono::high_resolution_clock::now().time_since_epoch().count());

    std::vector<std::pair<Zp, Zp>> result;
    for (const Zp &h_i : message)
    {
        uint32_t b_i = rnd() % (n - 1) + 1;
        b_i = 1;

        Zp first = pow(g, b_i);
        Zp second = h_i * pow(s, b_i);
...
int main()
{
    ...
    std::vector<Zp> message = scan_message(p);

    std::vector<std::pair<Zp, Zp>> result = encrypt_el_gamal(g, s, message);
