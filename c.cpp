...
std::vector<Fq> scan_message()
{
    ...
    std::vector<Fq> message;
    std::vector<Zp> coeffs;
    while (number > 0)
    {
        coeffs.push_back(Zp(number % p));
        if (coeffs.size() == n)
        {
            message.push_back(Fq(coeffs));
            coeffs.resize(0);
        }
        number /= p;
    }
...
std::vector<std::pair<Fq, Fq>> encrypt_el_gamal(const Fq& g, const Fq& s, const std::vector<Fq>& message)
{
    uint32_t n = g.get_ord();
    std::mt19937 rnd(std::chrono::high_resolution_clock::now().time_since_epoch().count());

    std::vector<std::pair<Fq, Fq>> result;
    for (const Fq &h_i : message)
    {
        uint32_t b_i = rnd() % (n - 1) + 1;

        Fq first = pow(g, b_i);
        Fq second = h_i * pow(s, b_i);
...
int main()
{
    ...
    std::vector<Fq> message = scan_message();

    std::vector<std::pair<Fq, Fq>> result = encrypt_el_gamal(g, s, message);
