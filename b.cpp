...
std::vector<Zp> decrypt_el_gamal(uint32_t a, const std::vector<std::pair<Zp, Zp>>& buffer)
{
    std::vector<Zp> result;
...
int main()
{
    ...
    std::vector<std::pair<Zp, Zp>> buffer;
    for (Zp r_i, m_i; std::cin >> r_i >> m_i;)
    {
        buffer.push_back({ m_i, r_i });
    }

    std::vector<Zp> result = decrypt_el_gamal(a, buffer);
